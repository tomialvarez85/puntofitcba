  ...
  return (
    <section className="bg-zinc-900 py-12">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">Qué dicen de nosotros</h2>
          <p className="mt-1 text-sm text-zinc-400">La opinión de quienes ya confiaron en nosotros.</p>
        </div>
      </div>
...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <div className="flex h-full flex-col rounded-2xl border border-zinc-700 bg-zinc-800 p-6 shadow-sm">
      <Quote size={28} className="text-brand-light" aria-hidden="true" />
      <p className="mt-3 line-clamp-4 flex-1 text-sm leading-relaxed text-zinc-300">{testimonial.review_text}</p>
      <div className="mt-5 flex items-center gap-3">
        {testimonial.photo_url ? (
          ...
          <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full bg-zinc-700 text-zinc-400">
            <User size={20} />
          </div>
        )}
