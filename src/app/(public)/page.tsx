    ...
    <div>
      ...
      {brands.length > 0 ? (
        <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-zinc-900 sm:text-3xl">Comprá por marca</h2>
            <p className="mt-1 text-sm text-zinc-500">Encontrá tu marca favorita.</p>
          </div>
      ...
      {promotions.length > 0 ? (
        <section className="bg-zinc-50 py-12">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-zinc-900 sm:text-3xl">Promociones activas</h2>
              <p className="mt-1 text-sm text-zinc-500">Descuentos por tiempo limitado.</p>
            </div>
...
function BrandCard({ brand }: { brand: Brand }) {
  return (
    <Link
      ...
      className="group flex w-24 flex-shrink-0 flex-col items-center gap-2 rounded-2xl border border-zinc-200 bg-white p-4 text-center shadow-sm transition hover:shadow-md sm:w-auto"
    >
      <div className="relative h-14 w-14">
        {brand.logo_url ? (
          <Image src={brand.logo_url} alt={brand.name} fill className="object-contain" />
        ) : (
          <div className="flex h-full w-full items-center justify-center rounded-lg bg-zinc-100 text-xs font-semibold text-zinc-400">
            {brand.name.slice(0, 2).toUpperCase()}
          </div>
        )}
      </div>
      <span className="line-clamp-1 text-xs font-medium text-zinc-700">{brand.name}</span>
    </Link>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-brand-light bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-lg font-semibold text-zinc-900">{promotion.name}</h3>
        <span className="rounded-full bg-brand px-3 py-1 text-xs font-bold uppercase tracking-wide text-white">
          {discountLabel}
        </span>
      </div>

      {promotion.description ? <p className="mt-1 text-sm text-zinc-500">{promotion.description}</p> : null}
        ...
        <div className="mt-4 flex gap-4 overflow-x-auto pb-1">
          {promotion.targets.map((target) => (
            <Link
              ...
              className="group w-36 flex-shrink-0 rounded-xl border border-zinc-200 bg-white p-3 transition hover:shadow-md sm:w-40"
            >
              ...
              <p className="mt-2 line-clamp-2 text-sm font-medium text-zinc-800">{target.name}</p>
              <div className="mt-1 flex items-baseline gap-2">
                <span className="text-xs text-zinc-400 line-through">${target.originalPrice.toFixed(2)}</span>
                <span className="text-sm font-bold text-brand">${target.discountedPrice.toFixed(2)}</span>
              </div>
            </Link>
          ))}
        </div>
